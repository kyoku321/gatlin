...
# Conventional language tag (BCP-47-ish), excluding path syntax.
_LANGUAGE_TAG_RE = re.compile(r"^[A-Za-z]{2,8}(?:[-_][A-Za-z0-9]{1,8})*$")
# Non-standard tags users commonly write, mapped to BCP-47 language tags.
_LANGUAGE_TAG_ALIASES = {"jp": "ja"}


def _normalize_language_tag(language: str) -> str:
    """Validate a language tag and normalize common non-standard aliases."""
    if not _LANGUAGE_TAG_RE.fullmatch(language):
        raise ValueError(f"invalid language code: {language!r}")
    return _LANGUAGE_TAG_ALIASES.get(language.lower(), language)
...
class AIConfig(BaseModel):
    ...
    max_tokens: int = 4096
    # Optional thinking-mode toggle for models that support it (e.g. Qwen3
    # served by vLLM). Sent as extra_body={"chat_template_kwargs":
    # {"enable_thinking": ...}} on OpenAI-compatible requests. None (default)
    # means the parameter is not sent and the server default applies.
    ...
    def validate_languages(cls, languages: List[str]) -> List[str]:
        ...
        Common non-standard tags are normalized to their BCP-47 form
        (e.g. "jp" -> "ja").
        """
        return [_normalize_language_tag(language) for language in languages]
...
class WebhookConfig(BaseModel):
    ...
    def validate_languages(cls, languages: Optional[List[str]]) -> Optional[List[str]]:
        """Normalize language filter tags the same way as ai.languages."""
