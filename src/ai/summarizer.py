...
_CJK = r"[\u4e00-\u9fff\u3400-\u4dbf\u3040-\u30ff\u31f0-\u31ff]"
_ASCII = r"[A-Za-z0-9]"
# CJK-based languages where spacing around ASCII letters/digits is expected.
_PANGU_LANGUAGES = {"zh", "ja"}


def _needs_pangu(language: str) -> bool:
    return language.lower() in _PANGU_LANGUAGES
...
LABELS = {
    ...
    "ja": {
        ...
        "references": "参考リンク",
        "tags": "タグ",
        "selected_items": "全 {total} 件のコンテンツから {selected} 件の重要ニュースを厳選しました。",
        "empty_analyzed": "{total} 件のコンテンツを分析しましたが、重要度しきい値に達する項目はありませんでした。",
        "empty_body": (
            "本日は特に重要な動きはありませんでした。考えられる理由：\n"
            "- 監視している情報源が静かな日だった\n"
            "- AI スコアのしきい値が高すぎる\n"
            "- 情報源の種類が不足している\n\n"
            "対応策：\n"
            "1. 現在の Profile のフィルタしきい値を下げる\n"
            "2. より多様な情報源を追加する\n"
            "3. AI モデルが正常に動作しているか確認する\n"
        ),
    },
}
...
class DailySummarizer:
    ...
    async def generate_summary(
        ...
        Args:
            ...
            language: Output language (e.g. "en", "zh", "ja")
    ...
    def generate_webhook_overview(
        ...
        if language == "zh":
            ...
            header = (
                f"# {labels['header']} - {date}\n\n"
                f"> 全 {total_fetched} 件中 {len(items)} 件の重要ニュースを厳選しました。\n\n"
                "各コンテンツの詳細を順に送信します。興味のあるタイトルだけご覧ください。\n\n"
            )
        else:
            header = (
                f"# {labels['header']} - {date}\n\n"
