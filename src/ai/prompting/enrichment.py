...
def target_language_instruction(language: str) -> str:
    """Canonical instruction phrase for the artifact target language.

    Known CJK languages also carry their own name so the directive is
    unambiguous to models that follow native-language instructions best.
    """
    return _TARGET_LANGUAGE_NAMES.get(
        language.lower(), f"language `{language}`"
    )


def target_language_section(language: str) -> str:
    """Prompt section stating the output-language contract.

    Explicitly decouples the artifact language from the source content
    language: models otherwise tend to mirror the language of the supplied
    article instead of switching to the requested language.
    """
    return f"""# Target language (required)

Write the complete artifact — the artifact title and every block title and content string — exclusively in {target_language_instruction(language)}.
The source content, comments, and tool results may be written in a different language. Translate and rewrite them into the target language; never output artifact text in the source content's language."""


def target_language_reminder(language: str) -> str:
    """Short reminder appended after the user message content.

    Re-asserts the output language at the response position, where models
    weight it most strongly; for CJK languages it is phrased in the target
    language itself.
    """
    parts = [
        f"Final reminder: the artifact language is {target_language_instruction(language)}."
    ]
    reminder = _TARGET_LANGUAGE_REMINDERS.get(language.lower())
    if reminder:
...
_TARGET_LANGUAGE_NAMES = {
    "zh": "Simplified Chinese (language tag `zh`, 简体中文)",
    "ja": "Japanese (language tag `ja`, 日本語)",
}
_TARGET_LANGUAGE_REMINDERS = {
    "zh": "请务必只使用简体中文输出标题与正文,不要输出源语言的句子。",
    "ja": "タイトルと本文は必ず日本語のみで記述してください。源内容の言語で出力しないでください。",
}
...
{target_language_section(language)}
