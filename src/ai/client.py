...
_SSL_VERIFY_FALSY = {"0", "false", "no", "off"}
_SSL_VERIFY_TRUTHY = {"1", "true", "yes", "on"}


def _resolve_ssl_verify() -> Optional[Union[bool, str]]:
    """Resolve TLS verification for AI API HTTP calls from HORIZON_SSL_VERIFY.

    - unset/empty: None (SDK default, full platform verification)
    - 0/false/no/off: False (skip certificate verification)
    - 1/true/yes/on: True (explicit full verification)
    - anything else: treated as a path to a CA bundle file

    This lets Horizon talk to endpoints that present self-signed
    certificates, e.g. an internal vLLM or Ollama server behind a
    Kubernetes ingress without a proper TLS certificate.
    """
    raw = os.getenv("HORIZON_SSL_VERIFY", "").strip()
    if not raw:
        return None
    lowered = raw.lower()
    if lowered in _SSL_VERIFY_FALSY:
        return False
    if lowered in _SSL_VERIFY_TRUTHY:
        return True
    return raw


def _build_ssl_http_client() -> Optional[AsyncHttpClient]:
    """Build an httpx client honoring HORIZON_SSL_VERIFY, or None for defaults.

    When HORIZON_SSL_VERIFY is not set the SDK-managed default client is
    used, so existing behavior is unchanged.
    """
    ...
    if verify is False:
        logger.warning(
            "HORIZON_SSL_VERIFY=%s: TLS certificate verification is DISABLED "
            "for AI API requests. Only use this for trusted internal "
            "endpoints (e.g. self-signed vLLM/Ollama servers).",
            os.getenv("HORIZON_SSL_VERIFY"),
        )
    elif isinstance(verify, str):
        # httpx >= 0.28 deprecates verify=<path>; pass an SSLContext instead.
        verify = ssl.create_default_context(cafile=verify)
    return AsyncHttpClient(
        verify=verify,
        # Match the OpenAI/Anthropic SDK default timeout (600s).
...
class AnthropicClient(AIClient):
    ...
    def __init__(self, config: AIConfig):
        ...
        http_client = _build_ssl_http_client()
    ...
    async def _do_request(
        ...
        *,
        ...
        if self.enable_thinking is not None:
            # Qwen3/vLLM-style thinking toggle. Modern vLLM expects it inside
            # chat_template_kwargs; a bare extra_body field is ignored.
