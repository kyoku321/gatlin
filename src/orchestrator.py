...
class HorizonOrchestrator:
    ...
    async def run(
        ...
        Args:
            ...
            date: Optional UTC date (YYYY-MM-DD); backfill mode fetches only
                content published on that day and writes the daily report for
                that date. Mutually exclusive with force_hours.
            notify: Whether to send webhook notifications; main.py defaults
                this to False in backfill mode so historical runs do not
                push stale digests.
        """
        self.console.print(
        ...
        try:
            ...
            if date:
                since, until = self._date_window(date)
            else:
                since = self._determine_time_window(force_hours)
            window_note = (
                f" until {until.strftime('%Y-%m-%d %H:%M:%S')} (UTC day {date})"
                if until
                else ""
            )
            self.console.print(
                f"{self.icons['date']} Fetching content since: "
                f"{since.strftime('%Y-%m-%d %H:%M:%S')}{window_note}\n"
            )
            ...
            if until is not None:
                all_items, dropped = self._filter_date_window(all_items, until)
                if dropped:
                    self.console.print(
                        f"{self.icons['filter']} Dropped {dropped} item(s) outside "
                        f"the {date} (UTC) window\n"
                    )
                if not all_items:
                    self.console.print(
                        "[yellow]No content within the requested date window. Exiting.[/yellow]"
                    )
                    return
            ...
            for lang in self.config.ai.languages:
                ...
                summary = await summarizer.generate_summary(important_items, report_date, len(all_items), language=lang)

                # Save to data/summaries/
                summary_path = self.storage.save_daily_summary(report_date, summary, language=lang)
                self.console.print(
                ...
                try:
                    ...
                    post_filename = f"{report_date}-summary-{lang}.md"
                    ...
                    front_matter = (
                        "---\n"
                        "layout: default\n"
                        f"title: \"Horizon Summary: {report_date} ({lang.upper()})\"\n"
                        f"date: {report_date}\n"
                        f"lang: {lang}\n"
                        "---\n\n"
                    )
                ...
                # Send email if configured (suppressed in backfill mode unless forced)
                if (
                    notify
                    ...
                    subject = f"Horizon Summary ({lang.upper()}) - {report_date}"
                    self.email_manager.send_daily_summary(summary, subject, subscribers)

                # Send webhook notification if configured
                if self.webhook_notifier and notify:
    ...
    @staticmethod
    def _date_window(date: str) -> tuple[datetime, datetime]:
        """UTC day boundaries for an inclusive calendar date (YYYY-MM-DD).

        Returns (since, until) where until is exclusive.
        """
    ...
    def _filter_date_window(
        items: List[ContentItem], until: datetime
    ) -> tuple[List[ContentItem], int]:
        """Keep only items published before ``until``.

        Items without a parseable published_at are dropped because their
        publication date cannot be verified.

        Returns the filtered list and the number of dropped items.
        """
        kept = [
            item
            for item in items
            if item.published_at is not None and item.published_at < until
        ]
