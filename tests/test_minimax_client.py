...
class TestEnableThinking:
    """Tests for the enable_thinking (Qwen3/vLLM) chat-template toggle."""
    ...
    def test_enable_thinking_false_sent_as_chat_template_kwargs(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "test-key")
        client = OpenAIClient(_make_config(
            provider=AIProvider.OPENAI,
            api_key_env="OPENAI_API_KEY",
    ...
    def test_enable_thinking_true_sent(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "test-key")
        client = OpenAIClient(_make_config(
            provider=AIProvider.OPENAI,
            api_key_env="OPENAI_API_KEY",
    ...
    def test_no_extra_body_when_enable_thinking_unset(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "test-key")
        client = OpenAIClient(_make_config(
            provider=AIProvider.OPENAI,
            api_key_env="OPENAI_API_KEY",
    ...
    def test_enable_thinking_survives_provider_chain_expansion(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "test-key")
        config = AIConfig(
            provider=AIProvider.OPENAI,
        ...
        assert all(
            cfg.enable_thinking is False for cfg in chained.configs
        )
