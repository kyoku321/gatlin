...
class TestFeishuCollapsibleJapanese:
    ...
    def test_overview_ja(self, monkeypatch):
        ...
        config = WebhookConfig(url_env=_TEST_URL_ENV, enabled=True)
        notifier = WebhookNotifier(config)

        text = notifier._build_feishu_collapsible_overview(
            item_count=3,
            all_items_count=40,
            date="2026-04-25",
            lang="ja",
        )
        assert "全 40 件中 3 件の重要ニュースを厳選しました" in text

        empty = notifier._build_feishu_collapsible_overview(
            item_count=0,
            all_items_count=40,
            date="2026-04-25",
            lang="ja",
        )
        assert "重要度しきい値に達するニュースはありませんでした" in empty
