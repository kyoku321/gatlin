...
# ── Japanese (ja) rendering ──


def test_generate_summary_ja_uses_localized_header_and_cjk_date():
    summarizer = DailySummarizer()
    item = _make_item(1)
    ...
    assert "> 全 10 件のコンテンツから 1 件の重要ニュースを厳選しました。" in result
    assert "rss · tester · 4月25日 08:00" in result
    assert "From 10 items" not in result
    assert "Apr 25, 08:00" not in result


def test_generate_summary_ja_uses_ja_profile_display_name():
    summarizer = DailySummarizer(
        profile_names={
            "tech-news": {
    ...
    assert "## テクノロジーニュース" in result
    assert "Technology News" not in result


def test_generate_summary_ja_applies_pangu_spacing():
    summarizer = DailySummarizer()
    item = _make_item(1)
    item.processing.artifacts["ja"] = ContentArtifact(
        language="ja",
        title="Qwen3.8モデルがリリースされた",
        blocks=[
            ContentBlock(
                id="summary",
                title="概要",
                content="新しいオープンソースLLMです。",
                primary=True,
            )
        ],
    )
    ...
    assert "Qwen3.8 モデルがリリースされた" in result
    assert "Qwen3.8モデル" not in result


def test_generate_webhook_item_ja_prefix():
    summarizer = DailySummarizer()
    item = _make_item(1)

    result = summarizer.generate_webhook_item(item, language="ja", index=2, total=5)

    assert result.startswith("全 5 件中 2 件目")
    assert "Item 2/5" not in result
    assert "第 2/5 条" not in result


def test_generate_webhook_overview_ja_header():
    summarizer = DailySummarizer()
    item = _make_item(1)
    ...
    assert "> 全 10 件中 1 件の重要ニュースを厳選しました。" in result
    assert "Selected 1 important items" not in result


def test_generate_empty_summary_ja_uses_localized_analyzed_line():
    summarizer = DailySummarizer()
    ...
    assert "> 10 件のコンテンツを分析しましたが、重要度しきい値に達する項目はありませんでした。" in result
    assert "Analyzed 10 items" not in result
