...
def test_enrichment_generates_blocks_and_validated_sources():
    ...
    assert "Simplified Chinese (language tag `zh`, 简体中文)" in requests[1]["system"]
    assert "never output artifact text in the source content's language" in requests[1]["system"]
    assert "简体中文" in requests[1]["user"]
