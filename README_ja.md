...
7. **配信（Deliver）** — 結果をメール、Feishuなどのwebhook、MCP、またはローカルファイルへ公開します。
...
| `--date YYYY-MM-DD` | — | バックフィル: 指定UTC日付に公開されたコンテンツのみ取得し、その日付の日報を書き出す(`--notify` なしなら Webhook はスキップ) |
| `--notify` | 無効 | `--date` バックフィルモードで Webhook 通知を送る |
...
| ガイド | 説明 |
|-------|-------------|
| [設定](docs/configuration.md) | AIプロバイダー、情報源、処理プロファイル、フィルタリング、メール、webhook、MCPのセットアップ |
...
Horizonはすでに日次ブリーフィングの全ループをサポートしています。マルチソース収集、プロファイル駆動の分析とエンリッチ、重複排除、コメント要約、2言語生成、メール配信、webhook配信、Dockerデプロイ、MCP統合、セットアップウィザードです。
