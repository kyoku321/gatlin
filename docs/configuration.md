...
### Thinking-mode toggle (Qwen3 / vLLM)

For thinking-capable models such as Qwen3 served by vLLM, disable the
internal reasoning pass with `ai.enable_thinking` in the active config file:
...
When set, it is sent on every OpenAI-compatible request as
`extra_body={"chat_template_kwargs": {"enable_thinking": ...}}` (the format
modern vLLM expects; the bare `enable_thinking` field is ignored by current
vLLM builds). When omitted, the parameter is not sent and the server default
applies. The field is accepted by all OpenAI-compatible providers
(`openai`, `ollama`, `ali`, `doubao`, `minimax`, `deepseek`) but only models
whose chat template supports `enable_thinking` actually use it.

### TLS certificate verification

When pointing `ai.base_url` at an internal endpoint that presents a
self-signed certificate (e.g. a vLLM or Ollama server behind a Kubernetes
ingress without a proper TLS certificate), configure `HORIZON_SSL_VERIFY`
in `.env`:

| Value | Behavior |
| --- | --- |
| unset (default) | Standard certificate verification |
| `0` (also `false`/`no`/`off`) | Skip certificate verification |
| `1` (also `true`/`yes`/`on`) | Explicit standard verification |
| `/path/to/ca-bundle.pem` | Verify against the given CA bundle |

```bash
# .env — skip TLS verification for the AI API only
HORIZON_SSL_VERIFY=0
```

This affects AI API requests (OpenAI-compatible, Anthropic, and Azure
clients) only, not source fetching. Prefer providing a CA bundle over
disabling verification entirely; disabling it is for trusted internal
endpoints only.
...
For webhook delivery, Horizon flattens HTML disclosure blocks such as `<details><summary>...</summary>` in `#{summary}` into plain Markdown link lists. This makes the generated summary easier to render in chat products. Saved Markdown files and email content are unchanged.
